...
var ErrUserUpdateProfile = errors.New("更新用户资料失败")
...
func (ud *GORMUserDAO) UpdateUserProfile(ctx context.Context, u User) error {
	now := time.Now().UnixMilli()
	err := ud.db.WithContext(ctx).Model(&u).Updates(&User{
		Name:     u.Name,
		Birthday: u.Birthday,
		Intro:    u.Intro,
		Utime:    now,

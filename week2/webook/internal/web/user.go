...
func (c *UserHandler) Edit(ctx *gin.Context) {
	...
	if len(req.Name) > NameMaxLen {
		ctx.String(http.StatusOK, "名称超过128位长度！")
		return
	}
	...
	//校验简介长度
	if len(req.Intro) > IntroMaxLen {
		ctx.String(http.StatusOK, "简介字数过多")
		return
	}
	...
	if err == service.ErrUserUpdateProfile {
		err = errors.New("用户资料更新失败")
		return
	}
...
// Profile 用户详情 , 假定id是通过浏览器传参，比较好测试。
func (c *UserHandler) Profile(ctx *gin.Context) {
	...
	if err != nil {
		ctx.String(http.StatusOK, "参数非法")
		return
	}
	u, err := c.svc.Profile(ctx, int64(id))

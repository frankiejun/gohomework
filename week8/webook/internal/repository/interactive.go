...
type InteractiveRepository interface {
	...
	GetTopN(ctx context.Context, n int) ([]domain.ArticlTopN, error)
}

type CachedReadCntRepository struct {
	cache     cache.InteractiveCache
	topNCache cache.ArticleTopnCache
...
func (c *CachedReadCntRepository) GetTopN(ctx context.Context, n int) ([]domain.ArticlTopN, error) {
	return c.topNCache.Get(ctx, n)
}
...
func NewCachedInteractiveRepository(dao dao.InteractiveDAO,
	cache cache.InteractiveCache, topNCache cache.ArticleTopnCache, l logger.LoggerV1) InteractiveRepository {

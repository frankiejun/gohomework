...
func (a *ArticleHandler) RegisterRoutes(s *gin.Engine) {
	...
	pub.POST("/topn)", ginx.WrapClaimsAndReq[TopnReq](a.GetTopN))
...
func (a *ArticleHandler) GetTopN(ctx *gin.Context, req TopnReq, uc jwt.UserClaims) (ginx.Result, error) {
	arts, err := a.intrSvc.GetTopN(ctx, req.Num)
	if err != nil {
		ctx.JSON(http.StatusOK, Result{
			Code: 5,
			Msg:  "系统错误",
